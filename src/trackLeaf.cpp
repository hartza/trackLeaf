...
trackLeaf::trackLeaf(ros::NodeHandle n) : firstLoop_(true), 
  calibration_on_(true), initialised_(false)
{
...
void trackLeaf::segmentNeighbors(const pcl::PointCloud<pcl::PointXYZ>& input_cloudXYZ_, pcl::PointCloud<pcl::PointXYZ>& output_cloudXYZ_)
{
  pcl::PointCloud<pcl::PointXYZ> tmp;
...
//   }
  ...
  if (inliers_.indices.size () == 0)
  {
    ...
    for (unsigned int i=0;i<cloudXYZ_p_.size();i++) {
      ...
      if (cloudXYZ_p_.points[i].z>max_z) max_z = cloudXYZ_p_.points[i].z;
      if (cloudXYZ_p_.points[i].z<min_z) min_z = cloudXYZ_p_.points[i].z;
    }
    
    //recompute position and size of the box as the new observation
    //put a maximum size
    box_size_(0)= (max_x -  min_x);
    if (box_size_(0)> MAX_X_BOX_SIZE) box_size_(0)= MAX_X_BOX_SIZE;
    box_size_(1)= (max_y -  min_y);
    if (box_size_(1)> MAX_Y_BOX_SIZE) box_size_(1)= MAX_Y_BOX_SIZE;
    box_size_(2)= (max_z -  min_z);
    if (box_size_(2)> MAX_Z_BOX_SIZE) box_size_(2)= MAX_Z_BOX_SIZE;
    ...
    br.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0, 0, 0)), ros::Time::now(), "my_frame", "kinect_depth"));
